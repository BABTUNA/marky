...
class PipelineDiagram(Flowable):
    ...
    def draw(self):
        ...
        for i, step in enumerate(self.pipeline_steps):
            ...
            canvas.roundRect(x, y - box_height, box_width, box_height, 4, fill=1, stroke=0)

            canvas.setFillColor(colors.white)
            canvas.setFont("Helvetica-Bold", 7)
            step_name = step.replace("_", " ").title()
            canvas.drawCentredString(x + box_width / 2, y - box_height / 2 - 3, step_name[:12])

            if i < len(self.pipeline_steps) - 1:
                canvas.setStrokeColor(color)
                canvas.setLineWidth(2)
                canvas.line(x + box_width, y - box_height / 2, x + box_width + 12, y - box_height / 2)
                canvas.setFillColor(color)
                canvas.line(x + box_width + 12, y - box_height / 2, x + box_width + 6, y - box_height / 2 - 4)
                canvas.line(x + box_width + 12, y - box_height / 2, x + box_width + 6, y - box_height / 2 + 4)
    ...
    async def run(
        ...
        try:
            ...
            heading_style = ParagraphStyle(
                ...
                spaceAfter=10, textColor=HexColor("#16213e"), fontName="Helvetica-Bold"
            )

            subheading_style = ParagraphStyle(
                "SubHeading", parent=styles["Heading3"], fontSize=11, spaceBefore=8,
                spaceAfter=5, textColor=HexColor("#0f3460"), fontName="Helvetica-Bold"
            )

            body_style = ParagraphStyle("Body", parent=styles["Normal"], fontSize=9, spaceBefore=3, spaceAfter=3)
            body_small = ParagraphStyle("BodySmall", parent=styles["Normal"], fontSize=7, spaceBefore=2, spaceAfter=2)
            quote_style = ParagraphStyle("Quote", parent=body_style, alignment=1, leftIndent=20, rightIndent=20)
            ...
            story.append(Paragraph(f"{duration}-Second {tone.title()} Advertisement", body_style))
            ...
            story.append(Paragraph(f"• YouTube Videos Analyzed: {summary.get('youtube_videos', 0)}", body_style))
            story.append(Paragraph(f"• Competitors Researched: {summary.get('competitors_found', 0)}", body_style))
            story.append(Paragraph(f"• Google Reviews Analyzed: {summary.get('google_reviews', 0)}", body_style))
            story.append(Paragraph(f"• Yelp Reviews Analyzed: {summary.get('yelp_reviews', 0)}", body_style))
            story.append(Paragraph(f"• Keywords Analyzed: {summary.get('keywords_analyzed', 0)}", body_style))
            ...
            if cost_data and not cost_data.get("error"):
                total = cost_data.get("summary", {}).get("total", 0) or cost_data.get("total", 0)
                if total:
                    story.append(Spacer(1, 0.15 * inch))
                    story.append(Paragraph(f"<b>Estimated Budget:</b> ${total:,}", body_style))
                    level = cost_data.get("summary", {}).get("budget_level", cost_data.get("budget_level", ""))
                    if level:
                        story.append(Paragraph(f"Budget Level: {level.title()}", body_style))

            if location_data and location_data.get("locations"):
                story.append(Paragraph(f"Potential Filming Locations: {len(location_data['locations'])}", body_style))

            if script_data:
                scenes = script_data.get("scenes", [])
                story.append(Paragraph(f"Script Structure: {len(scenes)} scenes", body_style))

            insights = research.get("insights", [])
            if insights:
                story.append(Spacer(1, 0.15 * inch))
                story.append(Paragraph("<b>Key Strategic Insights:</b>", subheading_style))
                for insight in insights[:6]:
                    clean = str(insight).replace("🎬", "").replace("✅", "").replace("❌", "").replace("💡", "").strip()
            ...
            pipeline_steps = ["research", "trend_analyzer", "script_writer", "image_generator",
                             "voiceover", "music", "video_assembly", "cost_estimator",
            ...
            for i, step in enumerate(pipeline_steps):
                ...
                story.append(Paragraph(f"{i+1}. <b>{step.replace('_', ' ').title()}</b>: {desc}", body_style))
            ...
            if local_intel:
                ...
                if top_comps:
                    story.append(Paragraph("Top Competitors in Target Market:", subheading_style))
                ...
                if what_top:
                    ...
                    story.append(Paragraph("✅ What Top Competitors Do Right:", subheading_style))
                ...
                if what_avoid:
                    story.append(Spacer(1, 0.1 * inch))
                    story.append(Paragraph("❌ Common Failure Patterns to Avoid:", subheading_style))
                ...
                if differentiators:
                    story.append(Spacer(1, 0.1 * inch))
                    story.append(Paragraph("💡 Recommended Differentiation Angles:", subheading_style))
            ...
            if google_reviews or yelp_reviews:
                ...
                if all_pain:
                    story.append(Paragraph("😤 Customer Pain Points to Address in Ad:", subheading_style))
                    for pain in all_pain[:8]:
                        clean = str(pain).get("point", pain) if isinstance(pain, dict) else pain
                        story.append(Paragraph(f"• {clean}", body_style))

                all_desires = google_reviews.get("desires", [])
                if all_desires:
                    story.append(Spacer(1, 0.1 * inch))
                    story.append(Paragraph("💭 Customer Desires & Wants:", subheading_style))
                    for desire in all_desires[:6]:
                        clean = str(desire).get("desire", desire) if isinstance(desire, dict) else desire
                        story.append(Paragraph(f"• {clean}", body_style))
                ...
                if all_praise:
                    story.append(Spacer(1, 0.1 * inch))
                    story.append(Paragraph("🏆 What Customers Love (Use as Proof):", subheading_style))
                    for praise in all_praise[:6]:
                        clean = str(praise).get("praise", praise) if isinstance(praise, dict) else praise
                        story.append(Paragraph(f"• {clean}", body_style))

                all_quotes = google_reviews.get("praise_quotes", []) + google_reviews.get("complaint_quotes", [])
                if all_quotes[:3]:
                    story.append(Spacer(1, 0.1 * inch))
                    story.append(Paragraph("💬 Actual Customer Quotes:", subheading_style))
            ...
            if trends:
                ...
                if timing:
                    story.append(Paragraph("📅 Seasonal Timing Recommendations:", subheading_style))
                    for rec in timing:
                ...
                if ad_recs:
                    story.append(Spacer(1, 0.1 * inch))
                    story.append(Paragraph("🎯 Ad Strategy Recommendations:", subheading_style))
            ...
            if related_questions:
                story.append(Paragraph("Content Intent & FAQ Opportunities", heading_style))
                story.append(Paragraph("Questions customers are asking (from Google 'People also ask'):", subheading_style))
            ...
            if viral_videos:
                story.append(Paragraph("Viral Ad Patterns & Reference Videos", heading_style))

                patterns = research.get("viral_patterns", {})
                hooks = patterns.get("common_hooks", [])
                if hooks:
                    story.append(Paragraph("🎬 Proven Hook Strategies:", subheading_style))
                    for hook in hooks[:6]:
                ...
                if styles:
                    story.append(Spacer(1, 0.1 * inch))
                    story.append(Paragraph("🎨 Effective Visual Styles:", subheading_style))
                ...
                if ctas:
                    story.append(Spacer(1, 0.1 * inch))
                    story.append(Paragraph("📢 Calls-to-Action That Convert:", subheading_style))
                    for cta in ctas[:5]:
                        story.append(Paragraph(f"• {cta}", body_style))

                story.append(Spacer(1, 0.1 * inch))
                story.append(Paragraph("Top Performing Reference Videos:", subheading_style))
                for i, video in enumerate(viral_videos[:5], 1):
                    title = video.get("title", "Unknown")
                    views = video.get("views", 0)
                    url = video.get("url", "")
                    story.append(Paragraph(f"{i}. <b>{title}</b> ({views:,} views)", body_style))
                    if url:
            ...
            if script_data:
                ...
                if script_text:
                    story.append(Paragraph(script_text.replace("---", "").strip(), body_style))
                    story.append(Spacer(1, 0.2 * inch))

                scenes = script_data.get("scenes", [])
                if scenes:
                    story.append(Paragraph("<b>Scene-by-Scene Breakdown:</b>", subheading_style))
                    for scene in scenes:
                        story.append(Paragraph(
                        ...
                        if visual:
                            story.append(Paragraph(f"   Visual: {visual[:150]}{'...' if len(visual) > 150 else ''}", body_small))
                ...
                if voiceover_text:
                    story.append(Spacer(1, 0.15 * inch))
                    story.append(Paragraph("<b>Full Voiceover Script:</b>", subheading_style))
            ...
            if frames:
                story.append(Paragraph(f"<b>{len(frames)} Visual Frames Generated:</b>", subheading_style))
                story.append(Spacer(1, 0.1 * inch))

                for i, frame in enumerate(frames, 1):
                    story.append(Paragraph(f"<b>Frame {i}</b> - {frame.get('timing', '')}", body_style))
                    desc = frame.get("description", "N/A")
                    story.append(Paragraph(f"<i>Concept:</i> {desc}", body_small))

                    prompt = frame.get("prompt", "")
                    if prompt:
                        story.append(Paragraph(f"<i>AI Prompt:</i> {prompt[:200]}{'...' if len(prompt) > 200 else ''}", body_small))

                    url = frame.get("url", "")
                    if url and url != "Not generated":
                        story.append(Paragraph(f"<i>Image URL:</i> {url[:80]}{'...' if len(url) > 80 else ''}", body_small))
            ...
            if cost_data and not cost_data.get("error"):
                ...
                if total:
                    story.append(Paragraph(f"<b>Total Estimated Budget: ${total:,}</b>", heading_style))
                    level = summary.get("budget_level", cost_data.get("budget_level", ""))
                    if level:
                        story.append(Paragraph(f"Budget Level: {level.upper()}", body_style))
                    story.append(Spacer(1, 0.15 * inch))
                ...
                if locations.get("line_items"):
                    story.append(Spacer(1, 0.1 * inch))
                    story.append(Paragraph("Location & Permit Costs:", subheading_style))
                ...
                if assumptions:
                    story.append(Spacer(1, 0.15 * inch))
                    story.append(Paragraph("<b>Production Assumptions:</b>", subheading_style))
                ...
                if schedule:
                    story.append(Spacer(1, 0.15 * inch))
                    story.append(Paragraph("<b>Production Schedule:</b>", subheading_style))
                    for day in schedule:
                        story.append(Paragraph(
                ...
                if tips:
                    story.append(Spacer(1, 0.15 * inch))
                    story.append(Paragraph("<b>💰 Budget Optimization Tips:</b>", subheading_style))
                    for tip in tips:
            ...
            if locations:
                ...
                for i, loc in enumerate(locations[:8], 1):
                    story.append(Paragraph(f"<b>{i}. {loc.get('name', 'Unknown')}</b>", subheading_style))
                    story.append(Paragraph(f"Address: {loc.get('address', 'N/A')}", body_style))
                ...
                if location_tips:
                    story.append(Paragraph("<b>📍 Location Scouting Tips:</b>", subheading_style))
            ...
            if trend_data and not trend_data.get("error"):
                ...
                if analysis:
                    ...
                    if hook:
                        story.append(Paragraph(f"<b>Recommended Hook:</b> {hook}", body_style))

                    visual = analysis.get("visual_style", "")
                    if visual:
                        story.append(Spacer(1, 0.1 * inch))
                        story.append(Paragraph(f"<b>Visual Style:</b> {visual}", body_style))

                    cta = analysis.get("cta", "")
                    if cta:
                        story.append(Spacer(1, 0.1 * inch))
                        story.append(Paragraph(f"<b>Call-to-Action:</b> {cta}", body_style))

                    key_msgs = analysis.get("key_messages", [])
                    if key_msgs:
                        story.append(Spacer(1, 0.1 * inch))
                        story.append(Paragraph("<b>Key Messages:</b>", subheading_style))
                    ...
                    if structure:
                        story.append(Spacer(1, 0.1 * inch))
                        story.append(Paragraph("<b>Recommended Structure:</b>", subheading_style))
            ...
            if voiceover and not voiceover.get("error"):
                ...
                story.append(Paragraph(f"File: {voiceover.get('audio_path', 'N/A')}", body_style))
            ...
            if music and not music.get("error"):
                ...
                story.append(Paragraph(f"File: {music.get('music_path', 'N/A')}", body_style))

            if video and not video.get("error"):
                story.append(Spacer(1, 0.1 * inch))
                story.append(Paragraph("🎬 Final Video Assembly:", subheading_style))
                story.append(Paragraph(f"File: {video.get('video_path', 'N/A')}", body_style))

            if social and not social.get("error"):
                captions = social.get("captions", {})
                if captions:
                    story.append(Spacer(1, 0.1 * inch))
                    story.append(Paragraph("📱 Social Media Captions:", subheading_style))
                    for platform, caption in captions.items():
                        story.append(Paragraph(f"<b>{platform.title()}:</b> {caption[:150]}...", body_small))
            ...
            story.append(Paragraph("This document was generated by AdBoard AI - Multi-Agent Advertising Platform", body_small))
