"""
Ad Intelligence Agent.
Analyzes competitor Google Ads (paid search) to extract strategies and patterns.
"""

import json
import os
import time
from collections import Counter
from datetime import datetime
from pathlib import Path
from typing import Any, Dict, List, Optional

from .models import AdAnalysis, AdData, AdPatterns, CompetitorAds
from .scraper import GoogleAdsScraper


class AdIntelAgent:
    """
    Ad Intelligence Agent.

    Scrapes and analyzes competitor Google Ads to extract:
    - Hook patterns (headlines)
    - Ad copy strategies
    - Trust signals
    - Messaging themes

    Uses SerpAPI (same key as local_intel).
    """

    def __init__(self, api_key: Optional[str] = None):
        self.scraper = GoogleAdsScraper(api_key)

    def analyze_market(
        self,
        business_type: str,
        location: str,
        max_ads: int = 50,
        additional_terms: Optional[List[str]] = None,
    ) -> AdAnalysis:
        """
        Analyze competitor ads in a market.

        Args:
            business_type: Type of business (e.g., "plumber")
            location: Location (e.g., "Providence RI")
            max_ads: Maximum ads to analyze
            additional_terms: Extra search terms

        Returns:
            Complete AdAnalysis
        """
        print(f"\n{'=' * 60}")
        print("Ad Intelligence Agent")
        print(f"{'=' * 60}")
        print(f"Business Type: {business_type}")
        print(f"Location: {location}")
        print(f"{'=' * 60}\n")

        start_time = time.time()

        # Build search queries - use broader terms to ensure we find ads
        search_terms = self._generate_search_queries(business_type, location)
        if additional_terms:
            search_terms.extend(additional_terms)

        # Step 1: Scrape Google Ads
        print("Step 1: Scraping Google Paid Ads (via SerpAPI)...")
        all_ads: List[AdData] = []

        # Calculate ads per query (minimum 5 to get useful results)
        ads_per_query = max(5, max_ads // len(search_terms))

        for term in search_terms:
            print(f"  Searching: '{term}'...")
            ads = self.scraper.search_ads(
                search_term=term,
                location=location,
                max_ads=ads_per_query,
            )
            print(f"    Found {len(ads)} ads")
            all_ads.extend(ads)
            time.sleep(0.5)  # Rate limiting for SerpAPI

        print(f"\n  Total ads found: {len(all_ads)}")

        if not all_ads:
            print("\n  No paid ads found - using industry best practices instead")
            return self._get_fallback_analysis(business_type, location)

        # Step 2: Group by advertiser
        print("\nStep 2: Grouping by advertiser...")
        competitor_ads = self._group_by_advertiser(all_ads)
        print(f"  Found {len(competitor_ads)} unique advertisers")

        for ca in competitor_ads[:5]:
            print(f"    - {ca.page_name}: {ca.total_ads} ads")

        # Step 3: Analyze patterns
        print("\nStep 3: Analyzing ad patterns...")
        patterns = self._analyze_patterns(all_ads)

        print(f"  Hook types: {len(patterns.hook_types)}")
        print(f"  CTA types: {len(patterns.cta_types)}")
        print(f"  Themes: {len(patterns.themes)}")

        # Step 4: Generate suggestions
        print("\nStep 4: Generating suggestions...")
        hook_suggestions = self._generate_hook_suggestions(patterns, business_type)
        headline_suggestions = self._generate_headlines(patterns, business_type)
        cta_suggestions = self._generate_cta_suggestions(patterns)
        creative_suggestions = self._generate_creative_suggestions(patterns)
        messaging_gaps = self._find_messaging_gaps(patterns, business_type)

        # Step 5: Analyze top vs bottom competitors
        print("\nStep 5: Analyzing top vs bottom competitors...")
        top_bottom = self._analyze_top_bottom_competitors(competitor_ads, all_ads)
        print(f"  Top performers: {len(top_bottom['top_competitors'])}")
        print(f"  Weak performers: {len(top_bottom['bottom_competitors'])}")

        elapsed = time.time() - start_time
        print(f"\n{'=' * 60}")
        print(f"Complete! ({elapsed:.1f}s)")
        print(f"{'=' * 60}\n")

        return AdAnalysis(
            business_type=business_type,
            location=location,
            search_query=search_terms[0],
            competitors_found=len(competitor_ads),
            total_ads_analyzed=len(all_ads),
            competitor_ads=competitor_ads,
            patterns=patterns,
            hook_suggestions=hook_suggestions,
            headline_suggestions=headline_suggestions,
            cta_suggestions=cta_suggestions,
            creative_suggestions=creative_suggestions,
            messaging_gaps=messaging_gaps,
            top_competitors=top_bottom["top_competitors"],
            what_top_competitors_do=top_bottom["what_top_competitors_do"],
            bottom_competitors=top_bottom["bottom_competitors"],
            what_to_avoid=top_bottom["what_to_avoid"],
        )

    def _group_by_advertiser(self, ads: List[AdData]) -> List[CompetitorAds]:
        """Group ads by advertiser/page."""
        by_page: Dict[str, List[AdData]] = {}

        for ad in ads:
            key = ad.page_name
            if key not in by_page:
                by_page[key] = []
            by_page[key].append(ad)

        competitor_ads = []

        for page_name, page_ads in by_page.items():
            # Aggregate patterns
            all_ctas = [ad.call_to_action for ad in page_ads if ad.call_to_action]
            all_hooks = [hook for ad in page_ads for hook in ad.hooks]

            media_mix = Counter(ad.media_type for ad in page_ads)

            competitor_ads.append(
                CompetitorAds(
                    page_name=page_name,
                    page_id=page_ads[0].page_id if page_ads else None,
                    total_ads=len(page_ads),
                    active_ads=sum(1 for ad in page_ads if ad.is_active),
                    ads=page_ads,
                    common_ctas=self._get_top_items(all_ctas, 3),
                    common_hooks=self._get_top_items(all_hooks, 3),
                    media_mix=dict(media_mix),
                )
            )

        # Sort by number of ads
        competitor_ads.sort(key=lambda x: x.total_ads, reverse=True)

        return competitor_ads

    def _analyze_patterns(self, ads: List[AdData]) -> AdPatterns:
        """Analyze patterns across all ads."""

        # Collect all patterns
        all_hooks = []
        all_ctas = []
        all_offers = []
        all_trust_signals = []
        all_emotions = []
        media_types = []

        for ad in ads:
            all_hooks.extend(ad.hooks)
            if ad.call_to_action:
                all_ctas.append(ad.call_to_action)
            all_offers.extend(ad.offers)
            all_trust_signals.extend(ad.trust_signals)
            all_emotions.extend(ad.emotional_appeals)
            media_types.append(ad.media_type)

        # Build pattern objects
        hook_counter = Counter(all_hooks)
        hook_types = [
            {
                "type": hook,
                "frequency": count,
                "percentage": count / len(ads) * 100 if ads else 0,
            }
            for hook, count in hook_counter.most_common(10)
        ]

        cta_counter = Counter(all_ctas)
        cta_types = [
            {
                "type": cta,
                "frequency": count,
                "percentage": count / len(ads) * 100 if ads else 0,
            }
            for cta, count in cta_counter.most_common(10)
        ]

        offer_counter = Counter(all_offers)
        offer_types = [
            {"type": offer, "frequency": count}
            for offer, count in offer_counter.most_common(10)
        ]

        # Media distribution
        media_counter = Counter(media_types)
        total_media = len(media_types)
        media_distribution = {
            media: count / total_media * 100 if total_media else 0
            for media, count in media_counter.items()
        }

        # Themes from emotions and trust signals
        themes = list(set(all_emotions + all_trust_signals))[:15]

        trust_signals = self._get_top_items(all_trust_signals, 10)

        return AdPatterns(
            hook_types=hook_types,
            cta_types=cta_types,
            offer_types=offer_types,
            media_distribution=media_distribution,
            themes=themes,
            trust_signals=trust_signals,
        )

    def _generate_hook_suggestions(
        self,
        patterns: AdPatterns,
        business_type: str,
    ) -> List[str]:
        """Generate hook suggestions based on patterns."""
        hooks = []

        # Based on common hook types
        hook_templates = {
            "[pain point]": f"Tired of unreliable {business_type}s?",
            "[desire]": f"Looking for a {business_type} you can trust?",
            "[need]": f"Need a {business_type} today?",
            "[command]": f"Stop calling {business_type}s who don't show up",
            "[warning]": f"Don't hire a {business_type} without reading this",
            "[relief]": f"Finally, a {business_type} who shows up on time",
            "[announcement]": f"Your local {business_type} is now accepting new customers",
            "[statistic]": f"Join 500+ satisfied customers this year",
        }

        for hook_type in patterns.hook_types[:5]:
            hook_name = hook_type.get("type", "")
            if hook_name in hook_templates:
                hooks.append(hook_templates[hook_name])

        # Add question hooks
        hooks.extend(
            [
                f"Still searching for a reliable {business_type}?",
                f"What if your {business_type} actually showed up on time?",
                f"Ready to stop dealing with overpriced {business_type}s?",
            ]
        )

        return hooks[:10]

    def _generate_headlines(
        self,
        patterns: AdPatterns,
        business_type: str,
    ) -> List[str]:
        """Generate headline suggestions."""
        headlines = []

        # From trust signals
        trust_to_headline = {
            "licensed": f"Licensed {business_type.title()}s You Can Trust",
            "insured": "Fully Insured for Your Protection",
            "experience": f"20+ Years of {business_type.title()} Experience",
            "local": f"Your Local {business_type.title()} Experts",
            "guarantee": "100% Satisfaction Guaranteed",
            "family owned": f"Family-Owned {business_type.title()} Service",
        }

        for signal in patterns.trust_signals[:5]:
            if signal in trust_to_headline:
                headlines.append(trust_to_headline[signal])

        # From themes
        theme_to_headline = {
            "speed": "Same-Day Service Available",
            "safety": "Safe & Reliable Service",
            "trust": "Trusted by Your Neighbors",
            "peace of mind": "Peace of Mind Guaranteed",
        }

        for theme in patterns.themes[:5]:
            if theme in theme_to_headline:
                headlines.append(theme_to_headline[theme])

        return headlines[:10]

    def _generate_cta_suggestions(self, patterns: AdPatterns) -> List[str]:
        """Generate CTA suggestions based on what's working."""
        ctas = []

        # Most common CTAs
        for cta_type in patterns.cta_types[:5]:
            ctas.append(cta_type.get("type", ""))

        # Standard high-performing CTAs
        standard_ctas = [
            "Call Now",
            "Get Free Quote",
            "Book Online",
            "Schedule Today",
            "Learn More",
            "Get Started",
        ]

        ctas.extend(standard_ctas)

        return list(dict.fromkeys(ctas))[:10]  # Unique, preserve order

    def _generate_creative_suggestions(self, patterns: AdPatterns) -> List[str]:
        """Generate creative format suggestions."""
        suggestions = []

        media_dist = patterns.media_distribution

        if media_dist.get("video", 0) > 40:
            suggestions.append(
                "Video ads dominate this market - prioritize video content"
            )
        else:
            suggestions.append("Image ads are common - video could stand out")

        if media_dist.get("carousel", 0) > 20:
            suggestions.append(
                "Carousel ads are popular - show before/after or multiple services"
            )

        # General recommendations
        suggestions.extend(
            [
                "Show real team members for trust",
                "Include before/after photos if applicable",
                "Add customer testimonial overlay",
                "Show your branded vehicle/uniform",
            ]
        )

        return suggestions[:8]

    def _find_messaging_gaps(
        self,
        patterns: AdPatterns,
        business_type: str,
    ) -> List[str]:
        """Find messaging opportunities competitors aren't using."""
        gaps = []

        # What's NOT in the patterns that could work
        potential_angles = [
            "emergency/24-7 service",
            "eco-friendly",
            "senior discounts",
            "financing available",
            "same-day service",
            "upfront pricing",
            "background-checked",
            "women-owned",
            "veteran-owned",
            "community involvement",
        ]

        used_themes = set(t.lower() for t in patterns.themes)
        used_signals = set(s.lower() for s in patterns.trust_signals)
        used = used_themes | used_signals

        for angle in potential_angles:
            if not any(word in used for word in angle.split()):
                gaps.append(f"No one emphasizes: {angle}")

        if len(gaps) < 3:
            gaps.extend(
                [
                    "Consider highlighting unique certifications",
                    "Showcase community involvement",
                    "Emphasize response time guarantees",
                ]
            )

        return gaps[:8]

    def _get_top_items(self, items: List[str], n: int) -> List[str]:
        """Get most common items."""
        counter = Counter(items)
        return [item for item, _ in counter.most_common(n)]

    def _analyze_top_bottom_competitors(
        self, competitor_ads: List[CompetitorAds], all_ads: List[AdData]
    ) -> Dict[str, List[str]]:
        """
        Analyze competitors to identify top performers vs weak performers.

        Top performers: Have strong trust signals, clear CTAs, emotional hooks
        Bottom performers: Weak/generic messaging, no differentiation
        """
        # Known top brands by industry (these are successful for a reason)
        top_brands = {
            "dominos",
            "pizza hut",
            "papa johns",
            "little caesars",
            "starbucks",
            "dunkin",
            "peets",
            "planet fitness",
            "anytime fitness",
            "la fitness",
            "equinox",
            "roto rooter",
            "mr rooter",
            "benjamin franklin plumbing",
            "aspen dental",
            "heartland dental",
            "merry maids",
            "molly maid",
            "the maids",
            "doordash",
            "uber eats",
            "grubhub",
            "great clips",
            "supercuts",
            "sport clips",
            "jiffy lube",
            "valvoline",
            "midas",
        }

        top_competitors = []
        bottom_competitors = []
        what_top_do = []
        what_to_avoid = []

        for comp in competitor_ads:
            name_lower = comp.page_name.lower()

            # Score the competitor based on ad quality signals
            score = 0
            strengths = []
            weaknesses = []

            # Check if it's a known successful brand
            is_known_brand = any(brand in name_lower for brand in top_brands)
            if is_known_brand:
                score += 3
                strengths.append("established brand with proven marketing")

            # Analyze their ads
            for ad in comp.ads:
                # Strong trust signals = good
                if ad.trust_signals:
                    score += len(ad.trust_signals)
                    for signal in ad.trust_signals[:2]:
                        if signal not in [s.split(":")[0] for s in strengths]:
                            strengths.append(f"uses '{signal}' trust signal")

                # Clear CTA = good
                if ad.call_to_action:
                    score += 1
                    if "clear CTA" not in str(strengths):
                        strengths.append(f"clear CTA: '{ad.call_to_action}'")

                # Emotional hooks = good
                if ad.emotional_appeals:
                    score += len(ad.emotional_appeals)
                    for appeal in ad.emotional_appeals[:1]:
                        strengths.append(f"emotional appeal: {appeal}")

                # Good hooks = good
                if ad.hooks:
                    score += 1

                # Check for weak signals
                if ad.ad_text:
                    text_lower = ad.ad_text.lower()
                    # Generic/weak messaging
                    if any(
                        weak in text_lower
                        for weak in ["click here", "buy now", "best price"]
                    ):
                        weaknesses.append(
                            "uses generic 'click here' or 'buy now' language"
                        )
                    # No differentiation
                    if len(ad.ad_text) < 50 and not ad.trust_signals:
                        weaknesses.append("ad copy too short, lacks substance")

            # Classify as top or bottom
            if score >= 3 or is_known_brand:
                top_competitors.append(comp.page_name)
                for strength in strengths[:2]:
                    insight = f"{comp.page_name}: {strength}"
                    if insight not in what_top_do:
                        what_top_do.append(insight)
            elif score <= 1 or weaknesses:
                bottom_competitors.append(comp.page_name)
                for weakness in weaknesses[:1]:
                    avoid = f"Avoid: {weakness} (seen in {comp.page_name})"
                    if avoid not in what_to_avoid:
                        what_to_avoid.append(avoid)

        # If we didn't find enough, add general insights
        if not what_top_do:
            what_top_do = [
                "Top brands use specific trust signals (licensed, insured, years in business)",
                "Successful ads have clear, action-oriented CTAs",
                "Best performers address customer pain points directly",
            ]

        if not what_to_avoid:
            what_to_avoid = [
                "Avoid generic messaging like 'best price' or 'click here'",
                "Don't use vague claims without proof",
                "Avoid overly long ad copy without clear benefits",
            ]

        return {
            "top_competitors": top_competitors[:5],
            "what_top_competitors_do": what_top_do[:5],
            "bottom_competitors": bottom_competitors[:3],
            "what_to_avoid": what_to_avoid[:5],
        }

    def _get_fallback_analysis(self, business_type: str, location: str) -> AdAnalysis:
        """Return useful ad insights even when no ads are found."""
        print("\n  Generating insights from industry best practices...")

        # Extract core term
        words = business_type.lower().split()
        core_term = (
            words[-1] if words else "business"
        )  # Usually the last word is most specific

        # Industry-specific hooks
        industry_hooks = {
            "coffee": [
                "Start your morning right",
                "Fuel your day",
                "Life's too short for bad coffee",
            ],
            "restaurant": [
                "Taste the difference",
                "Where flavor meets tradition",
                "Your table is waiting",
            ],
            "fitness": [
                "Transform your body",
                "Results you can see",
                "Your journey starts here",
            ],
            "plumber": ["Fast, reliable service", "No job too small", "Available 24/7"],
            "lawyer": [
                "Fighting for your rights",
                "Experience matters",
                "Free consultation",
            ],
            "default": [
                "Quality you can trust",
                "Experience the difference",
                "Your satisfaction guaranteed",
            ],
        }

        # Find matching hooks
        hooks = industry_hooks.get("default")
        for key, hook_list in industry_hooks.items():
            if key in business_type.lower():
                hooks = hook_list
                break

        # Create synthetic patterns based on best practices
        patterns = AdPatterns(
            hook_types=[
                {"type": "[pain point]", "frequency": 5, "percentage": 25},
                {"type": "[desire]", "frequency": 4, "percentage": 20},
                {"type": "[statistic]", "frequency": 3, "percentage": 15},
            ],
            cta_types=[
                {"type": "Call Now", "frequency": 6, "percentage": 30},
                {"type": "Get Quote", "frequency": 5, "percentage": 25},
                {"type": "Learn More", "frequency": 4, "percentage": 20},
            ],
            offer_types=[
                {"type": "free estimate", "frequency": 4},
                {"type": "limited time", "frequency": 3},
            ],
            media_distribution={"text": 60, "image": 30, "video": 10},
            themes=["trust", "quality", "local", "experience", "speed"],
            trust_signals=["licensed", "insured", "local", "reviews", "guarantee"],
        )

        return AdAnalysis(
            business_type=business_type,
            location=location,
            search_query=f"{business_type} {location}",
            competitors_found=0,
            total_ads_analyzed=0,
            competitor_ads=[],
            patterns=patterns,
            hook_suggestions=[
                f"Tired of unreliable {core_term} service?",
                f"Looking for a {core_term} you can trust?",
                hooks[0] if hooks else f"Experience the best {core_term} in town",
                hooks[1] if len(hooks) > 1 else f"Why settle for less?",
                f"Your neighbors already chose us",
            ],
            headline_suggestions=[
                f"Trusted Local {core_term.title()} Service",
                f"#1 Rated {core_term.title()} in {location.split(',')[0] if location else 'Town'}",
                "Quality Service, Fair Prices",
                "Satisfaction Guaranteed",
                "Family-Owned Since Day One",
            ],
            cta_suggestions=[
                "Call Now",
                "Get Free Quote",
                "Book Today",
                "Learn More",
                "Visit Us",
            ],
            creative_suggestions=[
                "Use authentic photos of your team/location",
                "Show before/after results if applicable",
                "Include customer testimonials",
                "Highlight what makes you unique",
            ],
            messaging_gaps=[
                "Emphasize your unique value proposition",
                "Highlight local community involvement",
                "Show social proof (reviews, ratings)",
                "Address common customer pain points",
            ],
            top_competitors=self._get_industry_top_brands(business_type),
            what_top_competitors_do=[
                "Use specific trust signals (licensed, insured, years in business)",
                "Have clear, action-oriented CTAs like 'Get Free Quote'",
                "Address customer pain points directly in headlines",
                "Include social proof and customer testimonials",
                "Show real team members and authentic imagery",
            ],
            bottom_competitors=[],
            what_to_avoid=[
                "Avoid generic messaging like 'best price' without proof",
                "Don't use vague claims - be specific about benefits",
                "Avoid stock photos - use authentic business imagery",
                "Don't hide pricing - transparency builds trust",
                "Avoid overly long ad copy without clear structure",
            ],
        )

    def _get_industry_top_brands(self, business_type: str) -> List[str]:
        """Get known top brands for an industry as examples to learn from."""
        industry_leaders = {
            "pizza": ["Domino's", "Pizza Hut", "Papa Johns"],
            "coffee": ["Starbucks", "Dunkin'", "Peet's Coffee"],
            "gym": ["Planet Fitness", "Anytime Fitness", "Equinox"],
            "fitness": ["Planet Fitness", "Orangetheory", "F45"],
            "plumber": ["Roto-Rooter", "Mr. Rooter", "Benjamin Franklin Plumbing"],
            "electrician": ["Mr. Electric", "Mister Sparky"],
            "hvac": ["One Hour Heating & Air", "Aire Serv"],
            "lawyer": ["Morgan & Morgan", "Cellino & Barnes"],
            "dentist": ["Aspen Dental", "Heartland Dental"],
            "cleaning": ["Merry Maids", "Molly Maid", "The Maids"],
            "restaurant": ["Chipotle", "Panera Bread", "Chick-fil-A"],
            "salon": ["Great Clips", "Supercuts", "Sport Clips"],
            "auto": ["Jiffy Lube", "Valvoline", "Midas"],
        }

        for key, brands in industry_leaders.items():
            if key in business_type.lower():
                return brands

        return ["Industry leaders in your space"]

    def _generate_search_queries(self, business_type: str, location: str) -> List[str]:
        """
        Generate search queries to find REAL competitor ads.

        Strategy:
        1. Search for national/big brands in the industry (they always run ads)
        2. Search for "best [industry]" which shows top competitors
        3. Search for industry + common buyer intent keywords
        """
        queries = []

        # Extract the core business type
        words = business_type.lower().split()
        core_term = words[-1] if words else business_type

        # Map to the PRIMARY industry keyword that big brands target
        industry_keywords = {
            # Food & Beverage
            "coffee": "coffee",
            "cafe": "coffee",
            "pizza": "pizza delivery",
            "restaurant": "restaurants",
            "food": "food delivery",
            "bakery": "bakery",
            "bar": "bar",
            # Services
            "plumber": "plumber",
            "plumbing": "plumber",
            "electrician": "electrician",
            "hvac": "hvac repair",
            "lawyer": "lawyer",
            "attorney": "personal injury lawyer",
            "dentist": "dentist",
            "doctor": "doctor",
            "cleaner": "cleaning service",
            "maid": "maid service",
            "landscap": "landscaping",
            "roofing": "roofing contractor",
            "contractor": "general contractor",
            # Fitness & Wellness
            "gym": "gym membership",
            "fitness": "gym",
            "yoga": "yoga classes",
            "personal train": "personal trainer",
            # Retail & Other
            "salon": "hair salon",
            "spa": "day spa",
            "auto": "auto repair",
            "car": "car dealership",
        }

        # Find the best matching industry keyword
        primary_keyword = None
        for key, kw in industry_keywords.items():
            if key in business_type.lower():
                primary_keyword = kw
                break

        if not primary_keyword:
            primary_keyword = core_term

        # STRATEGY 1: National brand searches (these ALWAYS have ads)
        national_brand_queries = {
            "pizza": ["dominos", "pizza hut", "papa johns pizza"],
            "coffee": ["starbucks coffee", "dunkin donuts"],
            "gym": ["planet fitness", "anytime fitness"],
            "plumber": ["roto rooter", "mr rooter"],
            "lawyer": ["morgan morgan lawyer", "injury lawyer"],
            "dentist": ["aspen dental", "dentist near me"],
            "cleaning": ["merry maids", "cleaning service"],
            "hvac": ["hvac service", "ac repair"],
            "restaurant": ["doordash", "uber eats", "grubhub"],
            "landscap": ["trugreen", "lawn care"],
            "auto": ["jiffy lube", "midas auto"],
            "salon": ["great clips", "supercuts"],
        }

        # Add brand queries for this industry
        for key, brands in national_brand_queries.items():
            if key in business_type.lower() or key in primary_keyword.lower():
                queries.extend(brands[:2])
                break

        # STRATEGY 2: High-intent buyer searches
        queries.extend(
            [
                f"{primary_keyword} near me",
                f"best {primary_keyword}",
                f"{primary_keyword} open now",
            ]
        )

        # STRATEGY 3: Problem/solution searches (high commercial intent)
        problem_queries = {
            "plumber": ["emergency plumber", "drain cleaning", "water heater repair"],
            "electrician": ["electrical repair", "electrician near me"],
            "hvac": ["ac not working", "furnace repair", "hvac emergency"],
            "lawyer": ["car accident lawyer", "injury attorney"],
            "dentist": ["emergency dentist", "teeth cleaning"],
            "auto": ["oil change", "brake repair", "car repair"],
            "pizza": ["pizza delivery", "order pizza online"],
            "coffee": ["coffee near me", "best coffee"],
            "gym": ["gym near me", "fitness center"],
            "restaurant": ["restaurants near me", "food near me"],
        }

        for key, problems in problem_queries.items():
            if key in business_type.lower() or key in primary_keyword.lower():
                queries.extend(problems[:2])
                break

        # STRATEGY 4: Location-specific
        city = location.split(",")[0].strip() if location else ""
        if city:
            queries.append(f"{primary_keyword} {city}")

        # Remove duplicates while preserving order
        seen = set()
        unique_queries = []
        for q in queries:
            q_lower = q.lower()
            if q_lower not in seen:
                seen.add(q_lower)
                unique_queries.append(q)

        return unique_queries[:12]  # More queries = more chances to find ads

    def save_report(
        self,
        analysis: AdAnalysis,
        output_dir: str = "output",
        filename: Optional[str] = None,
    ) -> str:
        """Save analysis to JSON file."""
        Path(output_dir).mkdir(exist_ok=True)

        if not filename:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"ad_intel_{timestamp}.json"

        filepath = Path(output_dir) / filename

        with open(filepath, "w", encoding="utf-8") as f:
            json.dump(analysis.to_dict(), f, indent=2, ensure_ascii=False)

        print(f"Report saved: {filepath}")
        return str(filepath)

    def print_summary(self, analysis: AdAnalysis):
        """Print summary of analysis."""
        print("\n" + "=" * 60)
        print("AD INTELLIGENCE SUMMARY")
        print("=" * 60)

        print(
            f"\nAnalyzed: {analysis.total_ads_analyzed} ads from {analysis.competitors_found} competitors"
        )

        if analysis.patterns:
            patterns = analysis.patterns

            print("\n### Media Mix")
            for media, pct in patterns.media_distribution.items():
                print(f"  - {media}: {pct:.0f}%")

            print("\n### Common CTAs")
            for cta in patterns.cta_types[:5]:
                print(f"  - {cta['type']} ({cta['frequency']} ads)")

            print("\n### Trust Signals Used")
            print(f"  {', '.join(patterns.trust_signals[:8])}")

            print("\n### Themes/Emotions")
            print(f"  {', '.join(patterns.themes[:8])}")

        print("\n### Hook Suggestions")
        for hook in analysis.hook_suggestions[:5]:
            print(f"  * {hook}")

        print("\n### Headline Suggestions")
        for headline in analysis.headline_suggestions[:5]:
            print(f"  * {headline}")

        print("\n### CTA Suggestions")
        for cta in analysis.cta_suggestions[:5]:
            print(f"  * {cta}")

        print("\n### Messaging Gaps (Opportunities)")
        for gap in analysis.messaging_gaps[:5]:
            print(f"  -> {gap}")

        print("\n" + "=" * 60)


def run_ad_analysis(
    business_type: str,
    location: str,
    max_ads: int = 50,
    save: bool = True,
    output_dir: str = "output",
) -> AdAnalysis:
    """
    Convenience function to run ad analysis.
    """
    agent = AdIntelAgent()

    analysis = agent.analyze_market(
        business_type=business_type,
        location=location,
        max_ads=max_ads,
    )

    agent.print_summary(analysis)

    if save:
        agent.save_report(analysis, output_dir)

    return analysis
