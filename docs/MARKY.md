...
python run_marky.py --cli -b "plumber" -l "Boston, MA" --drive-upload --drive-folder-id "YOUR_FOLDER_ID"
```
...
- **Orchestration:** Runs Local Intel → Review Intel → Yelp Intel → Trends Intel → Related Questions Intel sequentially, then optional PDF export and Google Drive upload
...
| 7 | PDF Export | Optional: report saved to `output/pdfs/` |
| 8 | Drive Upload | Optional: PDF uploaded to Google Drive (MCP) |
...
| PDF Export | ReportLab | report as PDF in `output/pdfs/` |
| Drive Upload | gdrive-pdf-upload-mcp-agent | PDF uploaded to Google Drive |
